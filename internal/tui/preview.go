...
func renderPreview(bookDir string, cfg *config.BookConfig, chNum int) (string, error) {
	...
	// Use glamour for terminal rendering
	style := "dark"
...
func RunPreview(bookDir string, cfg *config.BookConfig, chNum int) error {
	...
	// Detect RTL — glamour doesn't support RTL but we can warn
	...
	if err != nil {
		rendered = string(data) // fallback to raw
	}
	...
	// We need window size first; set content once we get it
	_ = rendered // set below after ready
