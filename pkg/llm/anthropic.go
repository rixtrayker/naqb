...
func (p *AnthropicProvider) Stream(ctx context.Context, model, system string, messages []Message, maxTokens int, onDelta StreamFunc) (string, error) {
	...
	if err := stream.Err(); err != nil && err != io.EOF {
		...
		return full, classifyAnthropicError(err)
	}
	log.Debug("LLM stream done", "model", model, "total_chars", len(full))
	return full, nil
}

// classifyAnthropicError converts a raw Anthropic SDK error to a typed sentinel.
// The SDK wraps HTTP errors in its own types, so we inspect the error string.
func classifyAnthropicError(err error) error {
	if err == nil {
		return nil
	}
	return ClassifyError("anthropic", 0, err.Error())
}
