...
func (p *OpenRouterProvider) Complete(ctx context.Context, model, system string, messages []Message, maxTokens int) (string, error) {
	...
	if orResp.Error != nil {
		return "", ClassifyError("openrouter", orResp.Error.Code, orResp.Error.Message)
	}
...
func (p *OpenRouterProvider) Stream(ctx context.Context, model, system string, messages []Message, maxTokens int, onDelta StreamFunc) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return "", ClassifyError("openrouter", resp.StatusCode, truncate(string(data), 300))
	}
	...
	for scanner.Scan() {
		...
		if chunk.Error != nil {
			return full.String(), ClassifyError("openrouter", chunk.Error.Code, chunk.Error.Message)
		}
