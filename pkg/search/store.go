...
func (s *Store) keywordSearch(_ context.Context, _ *chromem.Collection, query string, topK int) ([]SearchResult, error) {
	...
	for _, dir := range dirs {
		...
		for _, e := range entries {
			...
			score := 0
			contentWords := tokenizeWords(lower)
			for _, w := range words {
				// Check headings (lines starting with #)
				for _, line := range strings.Split(lower, "\n") {
					trimmed := strings.TrimSpace(line)
					if strings.HasPrefix(trimmed, "#") {
						headingWords := tokenizeWords(trimmed)
				...
				// Count whole-word body occurrences
...
// tokenizeWords splits text into lowercase words on non-letter/digit boundaries.
